...
def traceBackToStart(current_x, current_y, current_orient):
    """
    Trace back with anti-oscillation measures
    """
    x, y, orient = current_x, current_y, current_orient
    
    floodFillForReturn()
    log(f"Starting return journey from ({x}, {y}) to (0, 0)")
    API.setColor(0, 0, "G")
    
    move_count = 0
    max_moves = 500
    visited_positions = []  # **TRACK POSITION HISTORY**
    ...
    while (x, y) != (0, 0) and move_count < max_moves:
        ...
        if len(visited_positions) > 10:
            visited_positions.pop(0)  # Keep last 10 positions
            
        # Check for oscillation pattern
        if len(visited_positions) >= 4:
            if (visited_positions[-1] == visited_positions[-3] and 
                visited_positions[-2] == visited_positions[-4]):
                log(f"OSCILLATION DETECTED between {visited_positions[-1]} and {visited_positions[-2]}")
                # Force a different path by temporarily blocking current position
                floodFillForReturn()
        
        # Check walls at current position during return
        L = API.wallLeft()
        R = API.wallRight()  
        F = API.wallFront()
        ...
        # Check front wall before moving
        if API.wallFront():
            log(f"BLOCKED: Front wall detected at ({x}, {y}) during return!")
            abs_dir = ['n', 'e', 's', 'w'][orient]
            API.setWall(x, y, abs_dir)
            _record_wall(x, y, abs_dir)
            floodFillForReturn()
        ...
        try:
            ...
            if (x, y) == (0, 0):
                log(f"STARTING POINT REACHED! Stopping bot completely.")
                completeStop(0, 0, "COMPLETE")
                return True
    ...
    if (x, y) == (0, 0):
        ...
        log(f"Failed to return to start after {max_moves} moves")
        return False
...
def main():
    ...
    while not bot_terminated:
        ...
        if (x, y) in [(7, 7), (7, 8), (8, 7), (8, 8)]:
            ...
            success = traceBackToStart(x, y, orient)
            if success:
                log("RETURN JOURNEY COMPLETED - BOT STOPPED AT STARTING POINT")
                bot_terminated = True  # **COMPLETE TERMINATION**
                break
            else:
                log("Return journey failed!")
                bot_terminated = True  # **STOP ON FAILURE TOO**
                break
    ...
    log("BOT EXECUTION COMPLETED - ALL OPERATIONS STOPPED")
    API.clearAllText()
    if (x, y) == (0, 0):
        API.setText(0, 0, "COMPLETE")
    else:
        API.setText(x, y, "STOPPED")
